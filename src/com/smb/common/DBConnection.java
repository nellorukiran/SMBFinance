...
public class DBConnection {
	...
	public static Connection getDBConnection() {
		...
		if (con == null) {
			...
			try {
				...
				System.out.println("SQLException while DB Connection #####-1" + ex);
			} catch (Exception e) {
	...
	public static Connection getAWSDBConnection() {
		...
		try {
			Class.forName("com.mysql.cj.jdbc.Driver");
			awsCon = DriverManager.getConnection(jdbcUrl);
			System.out.println("JDBCDriver loaded! & AWS DB Connected successfully");
		} catch (ClassNotFoundException e) {
			throw new RuntimeException("Cannot find the driver in the classpath!", e);
		} catch (SQLException ex) {
			System.out.println("SQLException while AWS DB Connection #####-1 " + ex);
		} catch (Exception e) {
			System.out.println("Exception while AWS DB Connection #####-2 " + e);
		}
		return awsCon;
	}
