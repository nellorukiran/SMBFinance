...
var Moo3DCarousel = new Class({
	...
	*/
	...
	addElement: function(elem, size) {
		...
		// We store this size on the item.
		...
		// It adds the object to make transitions image
	...
	init: function(index) {
		...
		if(this._firstRun)
		{
			var currentGallery = (this._elements.length - this._currentIndex) % this._elements.length;
			this.changeCaption(currentGallery);
		...
		// It re-calculates the center of the elipse
		this._center = {
	...
	goTo: function(index) {
		...
		// We refinished our index of the image highlighted
