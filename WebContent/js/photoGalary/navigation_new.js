...
var navigation = function() {
	...
	function setStyles() {
		...
		if (tertiaryNavArray) {
			...
			maxDropHeight = parseInt(window.getSize().y / 2);
			...
			for (var i = 0; i < tertiaryNavArray.length; i++) {
				...
				for (var i = 0; i < spanArray.length; i++) {
					...
					if (currentList.getSize().y >= (maxDropHeight + tolerance) && currentList.getSize().y > minDropHeight) {
						currentList.setStyle("height", maxDropHeight + "px");
						currentList.setStyle("overflow", "hidden");
						setupSlider(currentList, i, maxDropHeight)
						if (currentList.getSize().x > currentSpan.getSize().x) {
							currentList.xWidth = currentList.getSize().x - 4
						} else {
							currentList.xWidth = currentSpan.getSize().x - 4
						}
					...
					if (currentList.getSize().x <= currentSpan.getSize().x) {
						currentList.setStyle("width", currentSpan.getSize().x + "px");
					}
				}
			}
		}
	...
	function onNavClick() {
		...
		function toggleNav(list) {
			...
			if (!list.hasClass("open")) {
				list.setStyle("top", currentSpan.getSize().y + "px");
	...
	function onNavItemClick() {
		...
		function setItem(itemHTML, parentSpan) {
			parentSpan.getChildren().set('html',itemHTML + '<img src="/img/decals/nav_arrow_ltbg.gif" />');
		}
